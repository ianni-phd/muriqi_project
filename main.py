...
def monoplot(
    ...
    title = "min_sample_leaf: {}, max_depth: {}".format(min_samples_leaf, max_depth)
    print(title)
    ...
    # MODELING
    # clf = DecisionTreeClassifier(min_samples_leaf=0, max_depth=999)   # PRIMA PROVA
    # clf = DecisionTreeClassifier(min_samples_leaf=2, max_depth=999)   # SECONDA PROVA
    ...
    plt.savefig(os.path.join(os.getcwd(), "output", title+"_Boundary.png"))

    fig = plt.figure(figsize=(25, 20))
    plt.title(title)
    _ = tree.plot_tree(clf, feature_names=["Shot", "Marking"], class_names="Class", filled=True)
    plt.savefig(os.path.join(os.getcwd(),"output",title+"_Tree.png"))
...
def multiplot(
    ...
    _ = plt.axis("tight")
    plt.savefig(os.path.join(os.getcwd(), "output", "Multi-plot.png"))
